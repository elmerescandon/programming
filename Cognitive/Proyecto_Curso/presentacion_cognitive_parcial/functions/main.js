...
const paginate = async()=>{
  ...
  let docs; // Varaibles solo están dentro de la función
  let postsRef = firebase.firestore().collection("posts").orderBy("title").startAfter(lastVisible).limit(3);
  ...
  docs["docs"].forEach((doc)=>{
    ...
    anchor.setAttribute("href","post.html#/"+doc.id); // Crear una página que tenga el post y el id
